...
public class Component<T> extends UnicastRemoteObject implements RMI<T>,
	...
	public void send(Message m, int receiverIndex) {
		...
		try {
			...
			RMI<T> reciever = (RMI<T>) registry.lookup(Integer
					.toString(receiverIndex));

			// send the message
			// reciever.receive(mObj);
			DelayedMessageSender<T> sender = new DelayedMessageSender<T>(
					reciever, mObj, 0);
	...
	public void processToken(Token token) {
		...
		synchronized (this) {
			System.out
					.println("componnet " + this.componentId
							+ "granted critical section!" + N(this.N) + " "
							+ S(this.S)+" in round "+this.index);
			...
			//System.out.println("self state" + this.componentId + N(N) + " "
			//		+ S(S));
	...
	public void gap() {
		...
		System.out.println(this.componentId + "want to sent new request!"+" in round"+this.index);
	}
	...
	public void processRequest(Request request) {
		synchronized (this) {
			System.out.println(this.componentId + "get request from"
					+ request.getSenderId());
			...
			case Other:
				...
				//System.out.println("process " + this.componentId
				//		+ "is in Other");
				break;
			case Execute:
				S[request.getSenderId()] = State.Request;
			...
			case Request:
				if (!S[request.getSenderId()].equals(State.Request)) {
					...
					//		+ "is Request");
					S[request.getSenderId()] = State.Request;
					send(new Request(this.componentId, N[this.componentId]),
							request.getSenderId());
				}
				break;
			case Hold:
				...
				S[this.componentId] = State.Other;
				System.out.println("process " + this.componentId
						+ "is holding and send to" + request.getSenderId());

				if (token == null) {
					System.out.println("token is null!");
				}
				token.setTSelement(request.getSenderId(), State.Request);// bug
																			// Hold
																			// but
																			// token
																			// is
																			// null
				token.setTNelement(request.getSenderId(),
						request.getNumOfRequest());

				send(newToken(token), request.getSenderId());
				System.out.println("process" + this.componentId
						+ "gives token to" + request.getSenderId() + " " + S(S)
						+ " " + N(N));
	...
	public void run() {
		...
		while (i < this.requestNum) {
			...
			if (S[this.componentId] != State.Hold
					&& S[this.componentId] != State.Execute
					&& sentRequest == false) {
				gap();
				this.request();
	...
	public void request() {
		synchronized (this) {
			if (this.componentId == 0)
				System.out.println("component" + this.componentId
						+ "request resource" + N(this.N) + " " + S(this.S));
